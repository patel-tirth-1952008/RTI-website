...
class PortalRouter:
    ...
    def __init__(self):
        ...
        # Map state name -> adapter instance
        self._state_adapters = {
            "gujarat": self._gujarat,
            "gj": self._gujarat,
        }

        # Official portal URLs for user-facing links
        self._portal_urls = {
    ...
    def get_adapter(
        ...
        1. Central subjects (NHAI, Railways, Passport, etc.) -> Central Portal
        2. Gujarat / AMC -> Gujarat State RTI Portal
        3. Other States -> Generic AI Adapter for that state's portal URL
        """
        ...
        # 3. Other states -> Generic adapter
    ...
    async def file_with_strict_jurisdiction(
        ...
        try:
            ...
            if not result.success:
                ...
                result.message = (
                    f"The {adapter.portal_name} could not be reached right now ({err}). "
    ...
    async def file_with_fallback(
        ...
        return await self.file_with_strict_jurisdiction(applicant, rti_request)
